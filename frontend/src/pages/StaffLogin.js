    ...
    <div className="max-w-md mx-auto px-4 py-10">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-slate-900">{t('auth.staffLogin')}</h1>
          <p className="text-slate-500 mt-2 text-sm">{t('auth.pickYourRole')}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="relative">
            ...
            <input
              required
              type="email"
              placeholder={role === 'admin' ? t('auth.adminEmail') : t('auth.volunteerEmail')}
          ...
          <div className="relative">
            ...
            <input
              required
              type="password"
              placeholder={role === 'admin' ? t('auth.adminPassword') : t('auth.volunteerPassword')}
          ...
          {role === 'admin' && (
            <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
              Demo admin credentials: {t('auth.adminEmail')} / {t('auth.adminPassword')}
            </div>
          )}
